...
# Predeclare for static analysis; actual class defined conditionally later
FallbackReportGenerator = None
...
# Detect Pillow availability separately for snippet/image rendering
try:
...
if not REPORTING_AVAILABLE:
    ...
    class FallbackReportGenerator:
        """Minimal fallback report generator used when reporting libs are missing.

        It writes simple text and JSON summaries instead of PDF/XLSX files so the
        rest of the system can run without installing heavy dependencies.
        """
        ...
        def _generate_test_assets(self, model_name, test_results):
            """Generate per-test human-readable assets (text fallback).

            Returns a list of Paths to created files.
            """
            images = []
            base = self.output_dir / 'images' / self.sanitize_filename(model_name)
            base.mkdir(parents=True, exist_ok=True)

            for r in test_results:
                test_id = r.get('test_id') or r.get('id') or 'unknown'
                # Only create assets for failed tests or when snippet/explanation present
                snippet = r.get('failure_snippet')
                explanation = r.get('failure_explanation')
                if not snippet and not explanation:
                    continue

                out_path = base / f"{test_id}.txt"
                with open(out_path, 'w', encoding='utf-8') as f:
                    f.write(f"Test ID: {test_id}\n")
                    f.write(f"Test Name: {r.get('test_name', '')}\n")
                    f.write("---\n")
                    if snippet:
                        f.write("Snippet:\n")
                        f.write(snippet + "\n\n")
        ...
        def generate_pdf_report(self, model_name, test_results, metadata=None, stats_by_category=None, stats_by_severity=None):
            safe_name = self.sanitize_filename(model_name)
            filename = self.output_dir / f"{safe_name}.pdf.txt"
        ...
        def generate_excel_report(self, model_name, test_results, metadata=None, stats_by_category=None, stats_by_severity=None):
            safe_name = self.sanitize_filename(model_name)
            filename = self.output_dir / f"{safe_name}.xlsx.json"
            payload = {
        ...
        def generate_reports(self, model_name, test_results, metadata=None, stats_by_category=None, stats_by_severity=None):
            pdf = self.generate_pdf_report(model_name, test_results, metadata, stats_by_category, stats_by_severity)
            excel = self.generate_excel_report(model_name, test_results, metadata, stats_by_category, stats_by_severity)
        ...
        def generate_comparison_report(self, all_model_results, test_suite, timestamp):
            filename_pdf = self.output_dir / "model_comparison.pdf.txt"
            filename_xlsx = self.output_dir / "model_comparison.xlsx.json"
            with open(filename_pdf, 'w', encoding='utf-8') as f:
                f.write(f"Model comparison report - {timestamp}\n")
                f.write(f"Total models: {len(all_model_results)}\n")
            with open(filename_xlsx, 'w', encoding='utf-8') as f:
                f.write(json.dumps({'timestamp': timestamp, 'models': [m['model_name'] for m in all_model_results]}))
            return {'pdf': filename_pdf, 'excel': filename_xlsx}

    # Expose the fallback as ReportGenerator by default; if real libs available,
    # the real implementation below will override this name.
    ReportGenerator = FallbackReportGenerator


if REPORTING_AVAILABLE:
    class ReportGenerator:
        """
        Service for generating evaluation reports in PDF and Excel formats (full-featured).
        """
        ...
        def _generate_test_assets(self, model_name, test_results):
            """Generate per-test PNG images (or text fallback) for failure snippets.
            ...
            # Use module-level IMAGE_AVAILABLE and PIL symbols
            pillow_ok = IMAGE_AVAILABLE
            ...
            for r in test_results:
                ...
                header_lines = [f"Test ID: {test_id}", f"Test Name: {r.get('test_name','')}"]
                body = ''
                if snippet:
                    body += "Snippet:\n" + snippet + "\n\n"
                if explanation:
                ...
                if pillow_ok:
                    ...
                    for hl in header_lines:
                        lines.extend(textwrap.wrap(hl, width=80))
                    lines.append('-' * 40)
                    ...
                    # Estimate height
                    line_height = 12
                    img_height = padding * 2 + line_height * (len(lines) + 2)
                    ...
                    txt_path = base / f"{test_id}.txt"
                    with open(txt_path, 'w', encoding='utf-8') as f:
                        for hl in header_lines:
                            f.write(hl + '\n')
                        f.write('\n')
                        f.write(body)
                    assets.append(txt_path)
    ...
    # Use the lightweight fallback implementation when reporting libs are missing
    ReportGenerator = FallbackReportGenerator
